...
from sqlalchemy import Text, Column, String, Integer, Float, DateTime, Boolean, ForeignKey, Enum, UniqueConstraint, Index
...
def add_configs( configs, log ):
  ...
  try:
    ...
    for config in configs:
      db_config = Config()
      attributes = [ "subsystem",  "name", "y_title", "plot_title", "metric", "relative_path", "histo1_path", "histo2_path", "reference_path", "threshold" ]
    ...
    for c1, c2 in zip(configs, configs_to_add): c1.db_id = c2.id
    # apply changes
    session.commit()
  except Exception as error_log:
    log.warning('failed to add config to the DB %s/%s, skip' % ( config.name, config.cfg_path ) )
...
def get_dataset( stream, reco_path, log):
  dataset = session.query( Dataset ).where(Dataset.stream == stream, Dataset.reco_path == reco_path).first()
  return dataset
...
def add_trends(dataset, trend_cfgs, log):
  log.info( 'Add new trends of dataset ("%s", "%s") to the DB ...' % (dataset.stream, dataset.reco_path) )
  trends_to_add = []
