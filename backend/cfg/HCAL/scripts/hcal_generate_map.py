...
file_name = [
    "Lmap_ngHB_N_20200212.txt",
    "Lmap_ngHE_N_20200212.txt",
    "Lmap_ngHF_N_20200212.txt",
    "Lmap_ngHO_N_20200212.txt",
]
        ...
        for line in in_file:
            if line[0] == "#":
                ...
                if replace_line[rbx_idx] in hcal_map_rbx_epd:
                    hcal_map_rbx_epd[replace_line[rbx_idx]].append(
                        (
                            int(replace_line[eta_idx]) * int(replace_line[side_idx]),
                            int(replace_line[phi_idx]),
                            int(replace_line[depth_idx]),
                        )
                    )
                else:
                    hcal_map_rbx_epd[replace_line[rbx_idx]] = [
                        (
                            int(replace_line[eta_idx]) * int(replace_line[side_idx]),
                            int(replace_line[phi_idx]),
                            int(replace_line[depth_idx]),
        ...
        if (
            ...
            if rbxname[0:2] == "HF":
                ...
                if eta < 0 and int(eta + 43) != 0:
                    hcal_map[rbxname].append((int(eta + 43), int(phi), int(dep)))
                elif eta > 0 and int(eta + 42) != 0:
                    hcal_map[rbxname].append((int(eta + 42), int(phi), int(dep)))
                else:
                    print("wierd! eta==0!")
        else:
            print(
                "wierd! eta, phi and dep out of range{0} {1}".format(
                    rbxname, (eta, phi, dep)
                )
            )
...
with open("hcal_metrics.py", mode="w") as out_map:
    ...
    out_map.write("}\n")
# mean time wrt rbx
with open("../trendPlotsHcal_DIGITiming_RBX.ini", mode="w") as out_DIGITiming_RBX, open(
    "../trendPlotsHcal_DIGITDCTime_RBX.ini", mode="w"
) as out_DIGITDCTime_RBX:
...
with open(
    "../trendPlotsHcal_DIGITiming_Depth.ini", mode="w"
) as out_DIGITiming_Depth, open(
    "../trendPlotsHcal_DIGITiming_Depth_ActiveChan.ini", mode="w"
) as out_DIGITiming_Depth_ActiveChan, open(
    "../trendPlotsHcal_DIGITDCTime_Depth.ini", mode="w"
) as out_DIGITDCTime_Depth, open(
    "../trendPlotsHcal_DIGITDCTime_Depth_ActiveChan.ini", mode="w"
) as out_DIGITDCTime_Depth_ActiveChan, open(
    "../trendPlotsHcal_DigiInfor_DeadChan.ini", mode="w"
) as out_DigiInfor_DeadChan:
    for get_key in sorted(set(hcal_map.keys())):
        if get_key != "":
            # depth 12
            out_DIGITiming_Depth.write(
                template_ini_DIGITiming_2Ddepth.format(
                    get_key, "12", (1, 2, 0), 1, 2, ""
                )
            )
            out_DIGITiming_Depth_ActiveChan.write(
                template_ini_DIGITiming_2Ddepthchann.format(
                    get_key, "12", (1, 2, 0), 1, 2, ""
                )
            )
            out_DIGITDCTime_Depth.write(
                template_ini_DIGITDCTime_2Ddepth.format(
                    get_key, "12", (1, 2, 0), 1, 2, ""
                )
            )
            out_DIGITDCTime_Depth_ActiveChan.write(
                template_ini_DIGITDCTime_2Ddepthchann.format(
                    get_key, "12", (1, 2, 0), 1, 2, ""
                )
            )
            out_DigiInfor_DeadChan.write(
                template_ini_DigiInfor_DeadChan.format(
                    get_key, "12", (1, 2, 0), 1, 2, ""
                )
            )
            # depth 34
            ...
            for get_key in sorted(set(hcal_map.keys())):
                if get_key[0:3] == sub_d[0:3]:
                    if get_key[0:2] == "HE" and dp == 56:
                        dp = 567
                    plot_name_DIGITiming_2Ddepth += (
                        plot_name_DIGITiming_2Ddepth_series.format(get_key, dp)
                    )
                    plot_name_DIGITiming_2Ddepthchann += (
                        plot_name_DIGITiming_2Ddepthchann_series.format(get_key, dp)
                    )
                    plot_name_DIGITDCTime_2Ddepth += (
                        plot_name_DIGITDCTime_2Ddepth_series.format(get_key, dp)
                    )
                    plot_name_DIGITDCTime_2Ddepthchann += (
                        plot_name_DIGITDCTime_2Ddepthchann_series.format(get_key, dp)
                    )
                    plot_name_DigiInfor_DeadChan += (
                        plot_name_DigiInfor_DeadChan_series.format(get_key, dp)
                    )
            plot_name_DIGITiming_2Ddepth = plot_name_DIGITiming_2Ddepth[0:-1] + "]"
            plot_name_DIGITiming_2Ddepthchann = (
                plot_name_DIGITiming_2Ddepthchann[0:-1] + "]"
            )
            plot_name_DIGITDCTime_2Ddepth = plot_name_DIGITDCTime_2Ddepth[0:-1] + "]"
            plot_name_DIGITDCTime_2Ddepthchann = (
                plot_name_DIGITDCTime_2Ddepthchann[0:-1] + "]"
            )
            plot_name_DigiInfor_DeadChan = plot_name_DigiInfor_DeadChan[0:-1] + "]"
            out_js.write(
                temp_js_DIGITiming_2Ddepth.format(
                    sub_d, dp, plot_name_DIGITiming_2Ddepth
                )
            )
            out_js.write(
                temp_js_DIGITiming_2Ddepthchann.format(
                    sub_d, dp, plot_name_DIGITiming_2Ddepthchann
                )
            )
            out_js.write(
                temp_js_DIGITDCTime_2Ddepth.format(
                    sub_d, dp, plot_name_DIGITDCTime_2Ddepth
                )
            )
            out_js.write(
                temp_js_DIGITDCTime_2Ddepthchann.format(
                    sub_d, dp, plot_name_DIGITDCTime_2Ddepthchann
                )
            )
            out_js.write(
                temp_js_DigiInfor_DeadChan.format(
                    sub_d, dp, plot_name_DigiInfor_DeadChan
                )
            )
