...
class TrendCfg():
  ...
  def GetMEs(self):
    answer = [self.relative_path, self.histo1_path, self.histo2_path, self.reference_path]
...
def compare_configs( new_config, old_config ):
  attributes = [ "y_title", "plot_title", "metric", "relative_path", "histo1_path", "histo2_path", "reference_path", "threshold" ]
  for attr in attributes:
    #print( attr, getattr(new_config, attr), getattr(old_config, attr) )
    if getattr(new_config, attr) != getattr(old_config, attr) : 
      print( new_config.cfg_path, type(getattr(new_config, attr)), type(getattr(old_config, attr)) )
      print( attr, getattr(new_config, attr), getattr(old_config, attr) )
      return 0
  return 1
