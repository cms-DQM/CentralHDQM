...
class BaseMetric:
    ...
    def __call__(self, histo, cacheLocation=None):
        if (
            not cacheLocation == None
            and not self.__cache == None
            and cacheLocation in self.__cache
        ):
            result, entries = self.__cache[cacheLocation]
        else:
            assert (
                not histo == None
            ), "reading from cache failed but no histo givento compute metric!"
            result = (0, 0)
            try:
        ...
        if entries < self._threshold:
            ...
            if VERBOSE:
                print(
                    " Number of entries (%s) is below threshold (%s) using '%s'"
                    % (entries, self._threshold, self.__class__.__name__)
                )
        if not len(result) == 2:
            raise (
                StandardError,
                "calculate needs to return a tuple with the value and the error of the metric!",
            )
        if not "__iter__" in dir(result[1]):
            ...
            if not len(result[1]) == 2:
                raise (
                    StandardError,
                    "when you use assymmetric errors you need to specify exactly two values. You gave: '%s'"
                    % result[1],
                )
    ...
    def calculate(self, histo):
        raise (
            StandardError,
            "you should not use the baseclass as a metric. Use the derived classes!",
        )
...
class MeanEntries(BaseMetric):
    def calculate(self, histo):
        return (
            histo.GetMean() * histo.GetEntries(),
            histo.GetMeanError() * histo.GetEntries(),
        )
...
class PixelEfficiency(BaseMetric):
    def calculate(self, histo):
        ...
        den = (
            histo.GetMean() * histo.GetEntries()
            + self._histo1.GetMean() * self._histo1.GetEntries()
        )
        if den == 0:
            res = 0
            eres = 0
...
class PixelDigiPerClusterPix(BaseMetric):
    def calculate(self, histo):
        ...
        if den == 0:
            ...
            eden = sqrt(
                self._histo1.GetMeanError() * self._histo1.GetMeanError()
                + self._histo2.GetMeanError() * self._histo2.GetMeanError()
            )
            if num == 0:
                eres = 0
            else:
                eres = res * sqrt(
                    (enum * enum) / (num * num) + (eden * eden) / (den * den)
                )
        if num < 1:
            res = 0
            eres = 0
...
class MeanRMS(BaseMetric):
    def calculate(self, histo):
        # if VERBOSE: print 'mean:',histo.GetMean(), histo.GetMeanError()
    ...
    def calculate(self, histo):
        ...
        for j in range(1, nbiny + 1):
            if histo.GetBinContent(1 + self.__modCounter, j) != 0:
                ...
                sumSquare += histo.GetBinContent(
                    1 + self.__modCounter, j
                ) * histo.GetBinContent(1 + self.__modCounter, j)
                count += 1
            if histo.GetBinContent(nbinx - self.__modCounter, j) != 0:
                summy += histo.GetBinContent(nbinx - self.__modCounter, j)
                sumSquare += histo.GetBinContent(
                    nbinx - self.__modCounter, j
                ) * histo.GetBinContent(nbinx - self.__modCounter, j)
                count += 1
...
class MeanDiff(BaseMetric):
    def calculate(self, histo):
        ...
        return (
            histo.GetMean() - self._reference.GetMean(),
...
class MaxExcursion(BaseMetric):
    def calculate(self, histo):
        ...
        err = sqrt(
            histo.GetBinError(bmax) * histo.GetBinError(bmax)
            + histo.GetBinError(bmin) * histo.GetBinError(bmin)
        )
        return (res, err)
...
class BinsCount(BaseMetric):
    ...
    def calculate(self, histo):
        ...
        sum = float(0.0)
        for bin in range(self.__loBin, histo.GetNbinsX() + 1):
            sum += histo.GetBinContent(bin)
        return (sum, sqrt(1 / sum) * sum if sum else 0)
...
class NormBinCount(BaseMetric):
    ...
    def calculate(self, histo):
        ...
        if frac.GetEntries() > total.GetEntries():
            raise (
                StandardError,
                " comparing '%s' to '%s' in '%s' makes no sense eff > 1!"
                % (self.__name, self.__norm, histo.GetName()),
            )
        ...
        if eff.GetN() < 1:
            raise (
                StandardError,
                "Efficiency cannot be calculated '%s' in '%s'"
...
class Ratio(BaseMetric):
    ...
    def calculate(self, histo):
        ...
        s = histo.Integral(histo.FindBin(self.__low), histo.FindBin(self.__high))
        T = histo.Integral()
        B = T - s
        return (
            s / B if B else 0,
...
class FractionInBin(BaseMetric):
    ...
    def calculate(self, histo):
        ...
        T = histo.GetEntries()
        return (s / T if T else 0, sqrt(1 / T + 1 / s) * s / T if T else 0)
...
class FractionInBinArray(BaseMetric):
    ...
    def calculate(self, histo):
        ...
        for bn in self.__binsnum:
            num += histo.GetBinContent(bn)
        for bd in self.__binsden:
            den += histo.GetBinContent(bd)
        return (
...
class BinRatio2D(BaseMetric):
    ...
    def calculate(self, histo):
        ...
        if den == 0:
            ...
            res = num / den
        return (res, res * sqrt((1.0 / den) + (1.0 / num)))
