    ...
    def __call__(self, histo, cacheLocation=None):
        if not cacheLocation == None and not self.__cache == None and cacheLocation in self.__cache:
            ...
            try:
                ...
                if VERBOSE: print("Warning: fit failed, returning 0")
        ...
        if entries < self._threshold:
            ...
            if VERBOSE: print(" Number of entries (%s) is below threshold (%s) using '%s'"%(entries, self._threshold, self.__class__.__name__))
        if not len(result) == 2:
...
class Fraction1(BaseMetric): 
    ...
    def calculate(self, histo):
        ...
        s = histo.Integral(histo.FindBin( self.__low),
                           histo.FindBin( self.__high))
        if VERBOSE: print("AAA",self.__high,self.__high+1)
        Nbins = histo.GetSize()
